...
func TestHandler_AddBooking(t *testing.T) {
	...
	validBody := RequestDto{
		...
		NumberOfGuests: 2,
		CheckInDate:    time.Date(2025, 9, 1, 0, 0, 0, 0, time.UTC),
		CheckOutDate:   time.Date(2025, 9, 10, 0, 0, 0, 0, time.UTC),
		CottageName:    primitive.NewObjectID().Hex(),
	}
	...
	t.Run("success returns 200", func(t *testing.T) {
		...
		svc.EXPECT().AddBooking(gomock.Any(), "A1", gomock.Any()).Return("id123", nil)
	...
	t.Run("service error returns 500", func(t *testing.T) {
		...
		svc.EXPECT().AddBooking(gomock.Any(), "A1", gomock.Any()).Return("", assertAnyError())
